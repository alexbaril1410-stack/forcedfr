...
def get_dashboard_stats() -> dict[str, Any]:
    with _db_connect() as conn:
        ...
        recent = conn.execute("SELECT action, details, created_at FROM torrent_actions ORDER BY created_at DESC LIMIT 8").fetchall()
    return {
...
<title>ForcedFR v2.5.3</title>
<style>
:root{color-scheme:dark;--bg:#0b0f14;--surface:#111720;--surface-2:#151d27;--surface-3:#1b2531;--border:#273241;--border-soft:#202a36;--text:#eef2f6;--muted:#8c98a7;--accent:#4f8cff;--accent-2:#3677ee;--green:#35c47a;--yellow:#e4ad48;--red:#ef6464;--shadow:0 12px 35px rgba(0,0,0,.18);font-family:Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}
*{box-sizing:border-box}html{background:var(--bg)}body{margin:0;background:radial-gradient(circle at 50% -15%,#182333 0,#0b0f14 38%);color:var(--text);min-height:100vh}main{max-width:1380px;margin:auto;padding:34px 30px 50px}h1,h2,h3,p{margin-top:0}h1{font-size:1.65rem;letter-spacing:-.03em;margin-bottom:3px}h2{font-size:1.18rem;letter-spacing:-.015em;margin-bottom:5px}h3{font-size:1rem}.sub,.small{color:var(--muted)}.sub{font-size:.9rem;line-height:1.45}.small{font-size:.82rem}
.app-header{display:flex;align-items:center;justify-content:space-between;gap:20px;margin-bottom:24px}.brand{display:flex;align-items:center;gap:13px}.brand-mark{display:grid;place-items:center;width:42px;height:42px;border-radius:12px;background:linear-gradient(135deg,var(--accent),#6f6cff);font-size:.78rem;font-weight:900;letter-spacing:-.04em;box-shadow:0 8px 24px rgba(79,140,255,.2)}.brand .sub{margin:0}.version{border:1px solid var(--border);background:rgba(17,23,32,.72);color:var(--muted);padding:7px 10px;border-radius:9px;font-size:.78rem;font-weight:700}
.services{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px;margin-bottom:24px}.service-card{display:flex;align-items:center;gap:11px;min-width:0;padding:13px 14px;background:rgba(17,23,32,.88);border:1px solid var(--border-soft);border-radius:12px;box-shadow:var(--shadow)}.service-icon{display:grid;place-items:center;width:32px;height:32px;flex:0 0 32px;border-radius:9px;background:var(--surface-3);color:#c7d2df;font-size:.7rem;font-weight:900}.service-name{font-size:.84rem;font-weight:800}.service-meta{margin-top:3px;font-size:.74rem}.status-dot{display:inline-block;width:7px;height:7px;border-radius:50%;margin-right:5px;background:var(--muted)}.status-dot.online{background:var(--green);box-shadow:0 0 0 3px rgba(53,196,122,.1)}
.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:11px}.card,.tablewrap,.settings-card{background:rgba(17,23,32,.9);border:1px solid var(--border);border-radius:14px;box-shadow:var(--shadow)}.card{padding:17px}.label{color:var(--muted);font-size:.73rem;text-transform:uppercase;letter-spacing:.06em;font-weight:800}.value{font-weight:750;margin-top:6px}.ok{color:var(--green)}.warn{color:var(--yellow)}.bad{color:var(--red)}
.tabs{display:flex;gap:5px;flex-wrap:wrap;margin:27px 0 22px;padding:5px;background:rgba(17,23,32,.86);border:1px solid var(--border);border-radius:13px;box-shadow:var(--shadow)}.tab,.subtab,button{font:inherit;border:1px solid transparent;border-radius:9px;padding:9px 13px;font-size:.83rem;font-weight:750;cursor:pointer;background:transparent;color:#b9c3cf;transition:background .16s,border-color .16s,color .16s,transform .16s}.tab:hover,.subtab:hover,button:hover{background:var(--surface-3);color:var(--text)}.tab.active,.subtab.active{background:#202b3a;border-color:#2c3a4b;color:#fff;box-shadow:0 3px 10px rgba(0,0,0,.16)}button:active{transform:translateY(1px)}button.primary{background:var(--accent-2)!important;border-color:#4b88f5!important;color:#fff!important}.panel{display:none}.panel.active{display:block}.toolbar,.filters,.subtabs{display:flex;gap:8px;flex-wrap:wrap}.toolbar{padding:14px;background:var(--surface);border:1px solid var(--border);border-radius:12px}.toolbar button{background:var(--surface-3);border-color:var(--border)}.progress{height:9px;background:#202a36;border-radius:999px;overflow:hidden;margin-top:13px}.progress div{height:100%;width:0;background:linear-gradient(90deg,var(--accent),#6f6cff);transition:.3s}.toolbar+.small,.progress+.small{display:block;margin-top:9px}
select,input{font:inherit;background:#0d131b;color:var(--text);border:1px solid var(--border);border-radius:9px;padding:9px 11px;outline:none}select:focus,input:focus{border-color:#477ee0;box-shadow:0 0 0 3px rgba(79,140,255,.1)}input{min-width:220px;flex:1}.filters{margin:12px 0}.filters select{min-width:180px}
.tablewrap{padding:0;overflow:auto;margin-top:13px}table{width:100%;border-collapse:collapse;min-width:650px}th,td{padding:13px 15px;text-align:left;border-bottom:1px solid var(--border-soft);vertical-align:middle}th{color:var(--muted);font-size:.7rem;text-transform:uppercase;letter-spacing:.06em;font-weight:800;background:#0f151d}tbody tr:hover{background:rgba(255,255,255,.018)}tbody tr:last-child td{border-bottom:0}.badge{font-weight:750}.yes{color:var(--green)}.no{color:var(--red)}.err{color:var(--yellow)}a.btn,.review-btn{display:inline-flex;align-items:center;justify-content:center;background:var(--surface-3);color:#dce5ee;text-decoration:none;padding:7px 10px;border:1px solid var(--border);border-radius:8px;font-size:.78rem;font-weight:700;margin:2px 4px 2px 0;white-space:nowrap}.review-btn:hover,a.btn:hover{background:#263447;border-color:#3a4c62;color:#fff}.empty{color:var(--muted);text-align:center;padding:28px}
.section-head{display:flex;align-items:flex-end;justify-content:space-between;gap:16px;margin:25px 0 13px}.section-head h2{margin:0}.section-head .sub{margin:5px 0 0}.activity{padding:0;overflow:hidden}.activity-row{display:grid;grid-template-columns:120px 180px minmax(0,1fr);gap:18px;align-items:center;padding:14px 17px;border-bottom:1px solid var(--border-soft)}.activity-row:last-child{border-bottom:0}.activity-date{color:var(--muted);font-size:.78rem;white-space:nowrap}.activity-action{font-weight:750;font-size:.82rem}.activity-details{color:#c2ccd7;font-size:.82rem;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.activity-dot{display:inline-block;width:7px;height:7px;border-radius:50%;background:var(--green);margin-right:8px;box-shadow:0 0 0 3px rgba(53,196,122,.08)}.activity-empty{padding:34px;text-align:center;color:var(--muted)}
.subtabs{margin:18px 0 14px;padding:4px;width:max-content;background:var(--surface);border:1px solid var(--border);border-radius:11px}.subtab{padding:8px 12px}.settings-grid{display:grid;grid-template-columns:minmax(290px,.8fr) minmax(0,1.5fr);gap:18px;align-items:start}.settings-card{padding:19px}.settings-card h3{margin:0 0 6px}.settings-card .desc{color:var(--muted);font-size:.84rem;line-height:1.5;margin:0 0 16px}.setting-item{display:flex;align-items:center;justify-content:space-between;gap:15px;padding:14px 0;border-top:1px solid var(--border-soft)}.setting-item:first-of-type{border-top:0}.setting-copy strong{display:block;font-size:.85rem}.setting-copy span{display:block;color:var(--muted);font-size:.76rem;margin-top:4px}.switch{position:relative;width:44px;height:24px;flex:0 0 auto}.switch input{display:none}.switch span{position:absolute;inset:0;background:#2b3542;border-radius:999px;cursor:pointer;transition:.2s}.switch span:before{content:"";position:absolute;width:18px;height:18px;left:3px;top:3px;background:#fff;border-radius:50%;transition:.2s}.switch input:checked+span{background:var(--green)}.switch input:checked+span:before{transform:translateX(20px)}.savebar{display:flex;justify-content:flex-end;margin-top:17px}.profile-list{display:grid;gap:12px}.profile-card{background:rgba(21,29,39,.76);border:1px solid var(--border);border-radius:13px;padding:17px}.profile-top{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:14px}.profile-name{font-size:.98rem;font-weight:850}.profile-type{color:var(--muted);font-size:.76rem;margin-top:3px}.profile-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.profile-field{display:flex;flex-direction:column;gap:7px}.profile-field label{color:var(--muted);font-size:.68rem;text-transform:uppercase;font-weight:800;letter-spacing:.05em}.profile-field select{width:100%;min-width:0}.profile-footer{display:flex;justify-content:space-between;align-items:center;gap:12px;margin-top:15px;padding-top:14px;border-top:1px solid var(--border-soft)}.status-pill{display:inline-flex;align-items:center;gap:6px;border-radius:999px;padding:5px 9px;background:#202a36;font-size:.72rem;font-weight:800}.status-pill.on{color:var(--green)}.status-pill.off{color:var(--muted)}.settings-title{margin-top:0;margin-bottom:12px}.settings-title h2{margin-bottom:4px}.settings-title p{margin:0}
#p-dashboard>.grid{margin-bottom:4px}#dashCards .card{min-height:105px}#dashCards .value{font-size:1.45rem;letter-spacing:-.03em}#p-errors>h2,#p-history>h2,#k-films>h2,#k-series>h2{margin-top:4px}.tablewrap+*{margin-top:0}
@media(max-width:1000px){.services{grid-template-columns:repeat(3,1fr)}.settings-grid{grid-template-columns:1fr}.profile-grid{grid-template-columns:repeat(3,1fr)}}@media(max-width:720px){main{padding:22px 15px 40px}.services{grid-template-columns:1fr 1fr}.app-header{align-items:flex-start}.tabs{overflow:auto;flex-wrap:nowrap}.tab{white-space:nowrap}.activity-row{grid-template-columns:1fr;gap:5px}.profile-grid{grid-template-columns:1fr 1fr}.profile-footer{align-items:flex-start;flex-direction:column}.savebar{justify-content:stretch}.savebar button{width:100%}}@media(max-width:480px){.services{grid-template-columns:1fr}.profile-grid{grid-template-columns:1fr}.version{display:none}}
</style></head><body><main>
<header class="app-header"><div><div class="brand"><span class="brand-mark">FF</span><div><h1>ForcedFR</h1><p class="sub">Surveillance des téléchargements et contrôle des bibliothèques.</p></div></div></div><div class="version">v2.5.3</div></header><p class="sub">Surveillance qBittorrent et contrôle des bibliothèques Radarr / Sonarr.</p>
<div class="services">
...
<div class="service-card"><div class="service-icon">QB</div><div><div class="service-name">qBittorrent</div><div class="service-meta value" id="qb">…</div></div></div>
<div class="service-card"><div class="service-icon">DC</div><div><div class="service-name">Discord</div><div class="service-meta value" id="discord">…</div></div></div>
<div class="service-card"><div class="service-icon">R</div><div><div class="service-name">Radarr</div><div class="service-meta value" id="radarr">…</div></div></div>
<div class="service-card"><div class="service-icon">S</div><div><div class="service-name">Sonarr</div><div class="service-meta value" id="sonarr">…</div></div></div></div>
